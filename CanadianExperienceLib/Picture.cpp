...
void Picture::Load(const wxString& filename)
{
    ...
    mMachine1->ChooseMachine(wxAtoi(root->GetAttribute(L"machine1-num", L"1")));
    mMachine2->ChooseMachine(wxAtoi(root->GetAttribute(L"machine2-num", L"2")));
    mMachine1->SetStart(wxAtoi(root->GetAttribute(L"machine1-start", L"0")));
    mMachine2->SetStart(wxAtoi(root->GetAttribute(L"machine2-start", L"0")));
...
/**
 * Bring up the machine choice dialog
 * @param parent Parent window for the dialog box
 * @param machine Machine number for the dialog box
 */
void Picture::MachineChoiceDialog(wxWindow* parent, int machine)
{
    switch(machine)
    {
    case 1:
        mMachine1->DoMachineDialog(parent);
        break;

    case 2:
        mMachine2->DoMachineDialog(parent);
        break;
    }
}
